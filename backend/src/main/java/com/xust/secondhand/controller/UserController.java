...
public class UserController {
    ...
    @PutMapping("/profile")
    public Result<UserResponse> updateProfile(@Valid @RequestBody UpdateUserRequest dto) {
        Long userId = UserContext.getUserId();
        UserResponse user = userService.updateProfile(userId, dto);
        return Result.success(user);
    }

    @Operation(summary = "我发布的商品")
    @GetMapping("/products")
    public Result<PageResult<ProductListResponse>> getUserProducts(
            @RequestParam(required = false) Integer status,
            @RequestParam(defaultValue = "1") int pageNum,
            @RequestParam(defaultValue = "10") int pageSize) {
        Long userId = UserContext.getUserId();
        PageResult<ProductListResponse> result = userService.getUserProducts(userId, status, pageNum, pageSize);
        return Result.success(result);
    }

    @Operation(summary = "我的收藏")
    @GetMapping("/favorites")
    public Result<PageResult<ProductListResponse>> getUserFavorites(
            @RequestParam(defaultValue = "1") int pageNum,
            @RequestParam(defaultValue = "10") int pageSize) {
        Long userId = UserContext.getUserId();
        PageResult<ProductListResponse> result = userService.getUserFavorites(userId, pageNum, pageSize);
        return Result.success(result);
    }
}
