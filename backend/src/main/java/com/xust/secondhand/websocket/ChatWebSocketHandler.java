...
import com.xust.secondhand.dto.request.WsMessageRequest;
import com.xust.secondhand.entity.ChatSession;
import com.xust.secondhand.entity.User;
import com.xust.secondhand.repository.ChatSessionRepository;
...
public class ChatWebSocketHandler extends TextWebSocketHandler {
    ...
    public ChatWebSocketHandler(ChatService chatService, ChatSessionRepository sessionRepository, UserRepository userRepository) {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        try {
            // 解析消息
            WsMessageRequest dto = objectMapper.readValue(message.getPayload(), WsMessageRequest.class);

            // 保存消息到数据库
            ChatMessageResponse savedMessage = chatService.saveMessage(senderId, dto.getSessionId(), dto.getContent(), dto.getType());

            // 获取会话信息，确定接收者
            ChatSession chatSession = sessionRepository.selectById(dto.getSessionId());
            if (chatSession == null) {
                return;
            }
